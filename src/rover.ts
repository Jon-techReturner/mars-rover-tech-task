...
const rotate = (turns: number) => (rover: Rover): ExecuteResult => {
...
const getNextPosition = (heading: Heading, position: Coordinates): Coordinates => {
    const [x, y] = position;
...
const outOfBounds = (next: Coordinates, max: Coordinates) => 
    next[0] < 0 || 
    next[1] < 0 || 
    next[0] > max[0] || 
    next[1] > max[1]
